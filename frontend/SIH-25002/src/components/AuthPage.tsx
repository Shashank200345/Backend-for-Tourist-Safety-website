...
// Updated TouristData interface to match the new structure
...
interface AuthPageProps {
  // No props needed - this is an authority portal for registering tourists
}
...
export const AuthPage: React.FC<AuthPageProps> = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Validate contact number (10 digits for Indian numbers starting with 6-9)
    const contactNumberRegex = /^[6-9]\d{9}$/;
    if (!contactNumberRegex.test(formData.contactNumber)) {
      setStatusMessage({ type: 'error', text: 'Contact number must be a valid 10-digit Indian mobile number (starting with 6-9).' });
      setIsSubmitting(false);
      return;
    }
    ...
    try {
      ...
      // Show success message - this is an authority portal, no user login needed
      setStatusMessage({ 
        type: 'success', 
        text: `Tourist registered successfully! Tourist ID: ${data.touristId}, Blockchain ID: ${data.blockchainId}` 
      });
    } catch (error: any) {
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // This is an authority portal - login is just for verification, not for user session
      setLoginStatus({ 
        type: 'success', 
        message: `Login verification successful. Tourist ID: ${data.user.touristId}, Blockchain ID: ${data.user.blockchainId}` 
      });
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen bg-crypto-bg text-crypto-text-primary relative overflow-hidden p-4 lg:p-6">
      {/* Background Effects */}
      <motion.div
            ...
            <div className="crypto-card p-6 lg:p-8 border border-crypto-border/60">
              <header className="flex flex-col gap-3 mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-[0.3em] text-crypto-text-secondary">Authority Portal</p>
                    <h2 className="text-2xl font-semibold gradient-text">Tourist Registration System</h2>
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <label className="flex flex-col gap-2 text-sm">
                    ...
                    <input
                      ...
                      required
                    />
                    <span className="text-xs text-crypto-text-secondary">10-digit Indian mobile number (starting with 6-9)</span>
                  </label>
                  <label className="flex flex-col gap-2 text-sm">
                    Date of Birth
                    <input
            ...
            <div className="crypto-card p-6 border border-crypto-border/60 space-y-4">
              ...
              {onboardingResult ? (
                ...
                <div className="text-sm text-crypto-text-secondary space-y-2">
                  <p>Complete the registration to store tourist details in the database with blockchain verification.</p>
                  <ul className="list-disc pl-5 space-y-1">
                    <li>Supports Aadhaar-based verification (UIDAI compliant hash storage)</li>
                    <li>Stores tourist data in Supabase database</li>
                    <li>Generates blockchain ID and transaction hash</li>
                    <li>Produces QR code for tourist identification</li>
                  </ul>
                </div>
              )}
            </div>
